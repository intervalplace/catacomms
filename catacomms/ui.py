"""Terminal view for catacomms. Standard library curses, same restraint as loraline.
...
from .engine import DIRECTIONS, PACK_LIMIT
...
SHRINE = ("\u00b7X\u00b7\u00b7", "XXX\u00b7", "\u00b7X\u00b7\u00b7", "\u00b7\u00b7\u00b7\u00b7")
WALL_COLOUR, FLOOR_COLOUR, LOOT_COLOUR, SHRINE_COLOUR = 238, 236, 179, 111
TRANSPARENT = -1
...
class DelveUI:
    ...
    def absorb(self, events) -> None:
        for event in events:
            ...
            if event.kind in ("hit", "death"):
                ...
                role = "warn"
            elif event.kind in ("end", "mend", "pickup", "shrine", "dropped"):
                role = "accent"
            if event.text:
    ...
    def _tile_grid(self, stdscr, left: int, main_w: int) -> int:
        ...
        def tile_at(gx: int, gy: int):
            ...
            if (gx, gy) in room.shrines:
                return SHRINE, SHRINE_COLOUR
    ...
    def _bar(self, entity, width: int = 10) -> str:
        cap = entity.hp_cap
        filled = 0 if cap <= 0 else entity.hp * width // cap
